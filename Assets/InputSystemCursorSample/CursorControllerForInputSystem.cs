...
namespace TSKT
{
    public class CursorControllerForInputSystem : InputActionUI
    {
        ...
        [SerializeField]
        InputActionReference axis = default;
        ...
        public override void Activate()
        {
            ...
            if (CurrentItem)
            {
                ...
                if (selectable)
                {
                    if (!CursorForInputSystem.Instance.IsMouseMode)
                    {
                        selectable.Select();
                    }
                }
        ...
        public override void Invoke(out bool exclusive)
        {
            ...
            if (axis.ToInputAction().phase == InputActionPhase.Started)
            {
                OnAxis(axis.ToInputAction().ReadValue<Vector2>());
            }
            exclusive = false;
        }
        ...
        bool Submit()
        {
            if (CursorForInputSystem.Instance.IsMouseMode)
            {
                CursorForInputSystem.Instance.IsMouseMode = false;
                return true;
            }
        ...
        void RefreshCursor()
        {
            if (CurrentItem)
            {
                ...
                CursorForInputSystem.Instance.SetFocus(
